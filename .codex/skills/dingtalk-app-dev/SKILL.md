...
- For robot interactive cards, model the delivery target explicitly as single chat, group chat, or both. Use `singleChatReceiver` for single-chat delivery and `openConversationId` for group-chat delivery. Confirm the group `openConversationId`; do not substitute a group name or ordinary group number.
...
- For lightweight message acknowledgement, use robot emotion reply `POST /v1.0/robot/emotion/reply` with `robotCode`, incoming `openConversationId`/`conversationId`, incoming `openMsgId`/`msgId`, `emotionName`, `emotionType`, and `textEmotion`. Keep this non-blocking: failures should be logged and must not interrupt the main bot workflow.
